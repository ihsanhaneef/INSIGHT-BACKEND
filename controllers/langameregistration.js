...
export const langameRegistration = async (req, res) => {
    try {
        ...
        const existingRegistration = await LangameRegistration.findOne({ email: req.body.email });

        if (existingRegistration) return res.status(400).json({ message: "Email is already registred." });

        const savedData = await LangameRegistration.create(req.body);
